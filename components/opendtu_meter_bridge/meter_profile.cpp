...
// Only the published instantaneous-measurement block is exposed. The DTSU666
// configuration and cumulative-energy blocks are intentionally excluded because
// the bridge has no authoritative values for them.
...
void encode_meter_profile(MeterProfile profile, const MeterMeasurements &measurements, uint16_t *registers,
  ...
  if (registers == nullptr || register_capacity == 0) {
    return;
  }
